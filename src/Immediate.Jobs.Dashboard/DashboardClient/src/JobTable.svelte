      ...
      <tbody>
        {#each rows as job}
          <tr>
            ...
            <td>
              {#if job.batchId}
                <button class="job-link" onclick={() => selectBatch(job.batchId)}>
                  {job.batchId}
                </button>
              {:else}
            ...
            <td>
              <span class="state {job.state.toLowerCase()}">{job.state}</span>
            </td>
            <td>{job.attempt}</td>
            <td>{fmt(job.dueAt)}</td>
            ...
            <td>
              <span class="actions">
                <button
                  ...
                  onclick={() => toggleDetails(job)}
                >
                  {isSelected(job) ? 'Hide' : 'Details'}
                </button>
                {#if job.state === 'Failed'}
                  <button
                    class="action"
                    onclick={() => act(`jobs/${encodeURIComponent(job.id)}/retry`)}
                  >
                    Retry
                  </button>
                  <button
                    class="action danger"
                    onclick={() => act(`jobs/${encodeURIComponent(job.id)}`, 'DELETE')}
                  >
                    Delete
                  </button>
                {/if}
              ...
              <td colspan="8">
                <section class="detail" aria-label={`Details for ${job.jobName}`}>
                  <div class="section-title">
                    <h2>{job.jobName}</h2>
                    <button class="action" onclick={() => select(null)}>Close</button>
                  </div>
                  <dl>
                    <div>
                      <dt>Invocation</dt>
                  ...
                  {#if job.context}
                    ...
                    <pre>{pretty(job.context)}</pre>
                  {/if}
                  {#if job.lastError}
                    <h3>Latest error</h3>
                    <pre class="failure">{job.lastError}</pre>
                  {/if}
                </section>
              </td>
            </tr>
