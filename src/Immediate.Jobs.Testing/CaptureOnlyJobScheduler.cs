...
public class CaptureOnlyJobScheduler<TPayload>(TimeProvider? timeProvider = null) : IJobScheduler<TPayload>
{
	...
	public virtual ValueTask<JobHandle> EnqueueAsync(TPayload payload, CancellationToken cancellationToken = default) =>
		CaptureAsync(payload, _timeProvider.GetUtcNow(), cancellationToken);
	...
	)
	{
		...
		return CaptureAsync(payload, _timeProvider.GetUtcNow() + delay, cancellationToken);
	}
	...
	public virtual ValueTask<JobHandle> ScheduleAtAsync(
		...
		CancellationToken cancellationToken = default
	) => CaptureAsync(payload, runAt, cancellationToken);
	...
	private ValueTask<JobHandle> CaptureAsync(TPayload payload, DateTimeOffset runAt, CancellationToken cancellationToken)
	{
