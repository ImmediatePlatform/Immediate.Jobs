...
internal static class QueueStorageConformance
{
	...
	)
	{
		...
		await storage.HeartbeatAsync(new()
		{
			...
			Acquisition = new() { LastAttemptedAt = liveAt, LastSucceededAt = liveAt, ItemsSucceeded = 3 },
			LeaseRenewal = new() { LastAttemptedAt = liveAt, LastFailedAt = liveAt, ConsecutiveFailures = 1, ItemsSucceeded = 1, ItemsFailed = 1 },
		}, cancellationToken);
